...
function Form({ attributes }) {
  ...
  const selectHandler = (e) => {
    ...
    if (name === "units" || name === "gpa" || name === "priorMember" || name === "permanentMember") {
      ...
      if (units === "1 - 11") {
        setWarningText(
          "You have to compolete 12 college or university units before Spring 2022 to be considered as an AGS member! You application needs to be reviewed by our advisor."
        );
      } else {
        setWarningText("");
      }
  ...
  const submitHandler = async (e) => {
    ...
    try {
      ...
      if (fileUploadCode !== "d7516f0c-9020-46f6-981b-1e3816e89d2c") {
        setSignatureWarning(
          `Your file upload confirmation code did not match. You will receive an upload confirmation code after submitting this Google form: ${transcriptUploadLink}`
        );
        return;
      } else {
        setSignatureWarning("");
...
function choiceProvider(type) {
  if (type === "unitsChoice") {
    ...
    return {
      name: "pmc",
      title: "We may form a Peer Mentoring Committee. Please choose one of the following, if applicable.",
      values: [
        "I have completed more than 30 units and am interested in being an AGS Peer Mentor",
        "I am new to AGS or SMC and am interested in being paired with an AGS Peer Mentor.",
        "Not Applicable",
      ],
    };
    ...
    return {
      ...
      title: "Which meeting time will you be able to join the general meeting on Thursday? *",
      values: ["11:15 AM to 12:35 PM", "6:00 to 7:20 PM"],
    };
